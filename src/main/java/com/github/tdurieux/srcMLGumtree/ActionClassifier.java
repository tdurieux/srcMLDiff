...
public class ActionClassifier {
	...
	// ROOT CLASSIFIER
	// /
	protected Set<ITree> srcUpdTrees = new HashSet<ITree>();

	protected Set<ITree> dstUpdTrees = new HashSet<ITree>();

	protected Set<ITree> srcMvTrees = new HashSet<ITree>();

	protected Set<ITree> dstMvTrees = new HashSet<ITree>();

	protected Set<ITree> srcDelTrees = new HashSet<ITree>();

	protected Set<ITree> dstAddTrees = new HashSet<ITree>();

	protected Map<ITree, Action> originalActionsSrc = new HashMap<ITree, Action>();
	protected Map<ITree, Action> originalActionsDst = new HashMap<ITree, Action>();
...
/*		for (Tree t : dstUpdTrees) {
		...
		for (ITree t : srcDelTrees) {
			if (!srcDelTrees.contains(t.getParent()) && !srcUpdTrees
					.contains(t.getParent())) {
				Action a = originalActionsSrc.get(t);
				rootActions.add(a);
		...
		for (ITree t : dstAddTrees) {
			if (!dstAddTrees.contains(t.getParent()) && !dstUpdTrees
					.contains(t.getParent())) {
				Action a = originalActionsDst.get(t);
				rootActions.add(a);
			}
